...
od <- options( digits=7)
on.exit( options( od))
print( load( 'samp_delfi_D.rda')) # object samp_notog1
...
tru_Nadf_Y <- sumover( samp_delfi_D@secret$N_sya[SLICE='F', Brange, Amat:40], 'A')

maxyplot <- max( c( tru_Nadf_Y, env$N_Y)) * 1.05
plot( Brange, tru_Nadf_Y, type='l', ylim=c( 0, maxyplot), ylab='Nadf_Y_Y')
