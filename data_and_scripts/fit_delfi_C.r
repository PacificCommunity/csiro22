...
od <- options( digits=7)
on.exit( options( od))
print( load( 'samp_delfi_C.rda')) # object samp_notog1
...
tru_Nadf_Y <- sumover( samp_delfi_C@secret$N_sya[SLICE='F', Brange, Amat:40], 'A')

maxyplot <- max( c( tru_Nadf_Y, env$N_Y)) * 1.05
plot( Brange, tru_Nadf_Y, type='l', ylim=c( 0, maxyplot), ylab='Nadf_Y')
...
evalq( envir=env2, { # will happen "live" inside env2
  names( dimnames( n_MOP_BYF))[3] <- 'A' # instead of "Fuzzage"
  names( dimnames( n_comp_MOP_BYF))[3] <- 'A'
  ...
  n_MOP_BYA <- n_MOP_BYF * OK
  n_comp_MOP_BYA <- n_comp_MOP_BYF * OK
})
...
maxyplot <- max( c( maxyplot, env2$N_Y)) * 1.05
plot( Brange, tru_Nadf_Y, type='l', ylim=c( 0, maxyplot), ylab='Nadf_Y')
points( Brange, env$N_Y, col='green', pch=16)
points(Brange, env2$N_Y, col = "orange", pch = 16)
