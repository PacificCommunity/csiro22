...
od <- options( digits=7)
on.exit( options( od))
print( load( 'samp_rhombi_D.rda')) # object samp_notog1
...
env$Nad_S # estimated
cat( 'Tru:\n')
samp_rhombi_D@secret$Nad_S # trooo

# Compare with bleedin' obvious...
Nobvious <- with( env, n_comp_POP_S / n_POP_S)

# How would aggregate version go?
Nsexlumpo <- with( env, 2 * sum( n_comp_POP_S) / sum( n_POP_S))
sum( Nobvious)
