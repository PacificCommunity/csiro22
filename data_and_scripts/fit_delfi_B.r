...
od <- options( digits=7)
on.exit( options( od))
print( load( 'samp_delfi_B.rda')) # object samp_notog1
...
tru_Nadf_Y <- sumover( samp_delfi_B@secret$N_sya[SLICE='F', Brange, Amat:40], 'A')

maxyplot <- max( c( tru_Nadf_Y, env$N_Y))
...
env2$n_MOP_BYA <- env2$n_MOP_noHands_BYA
env2$n_comp_MOP_BYA <- env2$n_comp_MOP_noHands_BYA
